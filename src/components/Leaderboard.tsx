    ...
    <Paper sx={{ p: 4, maxWidth: '80%', mx: 'auto', boxShadow: 3, borderRadius: 2 }}>
      ...
      <div className="flex flex-col gap-4">
        <LeaderboardSection
          ...
          setSelectedDate={setSelectedDailyDate}
          loading={loadingDaily}
        />
        <LeaderboardSection
          title="Weekly Leaderboard"
          data={weeklyLeaderboard}
          setSelectedDate={setSelectedWeeklyDate}
    ...
    <div className="bg-blue-50 p-6 rounded-xl shadow-md border border-blue-200">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex justify-end gap-4 ">
          {['Daily Leaderboard', 'Weekly Leaderboard'].includes(title) && setSelectedDate && (
            <DatePickerDemo isWeek={title === 'Daily Leaderboard' ? false : true} setSelectedDate={setSelectedDate} />
          )}
                  ...
                  {data.length > 0 ? (
                    data.map((user, index) => (
                      <TableRow key={user.id} sx={{ bgcolor: index % 2 === 0 ? '#F0F4FF' : 'white' }}>
                        ...
                        <TableCell>
                          {user.game_id.length > 10 ? user.game_id.slice(0, 10) + '...' : user.game_id}
                        </TableCell>
